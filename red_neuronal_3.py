...
model.add(Conv2D(kernel_size=2,strides=2,filters=16,padding="same",activation="elu",name="capa_1"))
model.add(MaxPool2D(pool_size=1,strides=2))

model.add(Conv2D(kernel_size=3,strides=1,filters=16,padding="same",activation="elu",name="capa_2"))
...
model.compile(optimizer="Nadam",loss="MeanSquaredError", metrics=["Recall"])

#entrenamiento
model.fit(x=imagenes, y=probabilidades, epochs=30,batch_size=30)
...
informacion = model.compile(optimizer="Nadam",loss="MeanSquaredError",metrics=['Recall',tf.keras.metrics.Precision(), tf.keras.metrics.Recall()])
print(informacion)
